...
function fileInclude() {
  return gulp
    ...
    .pipe(
      replace(
        new RegExp('(\\.+\\/)+' + additionNames.js + '/.*\\.js', 'g'),
        function (match) {
          path = match.replace(/(\.+\/)+/, '')
      ...
      replace(
        ...
        function (match, p1) {
          ...
          return match.replace(
            'node_modules',
            paths.dist.vendor.dir.replace(paths.dist.base.dir + '/', '')
          )
        }
      )
    )
...
function CSS() {
  return gulp.src(paths.src.css.files).pipe(gulp.dest(paths.dist.css.dir))
}
...
function copyNode() {
  ...
  if ([...node].length) {
    return gulp.src([...node]).pipe(gulp.dest(paths.dist.vendor.dir))
  } else {
    return new Promise(function (resolve, reject) {
      resolve()
    })
...
function copySkipedFiles() {
  ...
  if ([...skipedFiles].length) {
    return gulp.src([...skipedFiles]).pipe(gulp.dest(paths.dist.base.dir))
  } else {
    return new Promise(function (resolve, reject) {
      resolve()
    })
...
function copyDependencies() {
  for (var k in config.copyDependencies.dist) {
    ...
    gulp
      .src(path)
      .pipe(
        gulp.dest(paths.dist.base.dir + '/' + config.copyDependencies.dist[k])
      )
  }
