  ...
  return gulp
    ...
    .pipe(
      replace(
        new RegExp(
          '(.*?)(\\.+\\/)+' +
            `(${additionNames.vendor}|node_modules)` +
            '/.*\\.(js|css)',
          'g'
        ),
        function (match, p1, p2, p3, p4) {
          ...
          if (path.search('node_modules') === 0) {
            ...
            return match.replace(
              'node_modules',
              paths.build.vendor.dir.replace(paths.build.base.dir + '/', '')
            )
          }
    ...
    .pipe(
      replace(
        new RegExp(
          '(.*?)(\\.+\\/)+' +
            `(${additionNames.js}|${additionNames.css})` +
            '/.*\\.(js|css)',
          'g'
        ),
        function (match, p1, p2, p3, p4) {
    ...
    .pipe(
      deleteLines({
        filters: [new RegExp(config['deleteLine:build'], 'i')],
      })
    )
    ...
    .pipe(
      replace(
        /<!-- bundlecss:vendor \[(.*?)\](.*)-->/g,
        function (math, p1, p2) {
          return `<link rel="stylesheet" href="${p1}/${paths.build.build.css}/${
    ...
    .pipe(
      replace(
        /<!-- bundlejs:vendor \[(.*?)\](.*)-->/g,
        function (math, p1, p2) {
          return `<script src="${p1}/${paths.build.build.js}/${
...
function copeImages() {
  ...
  if ([...images].length) {
    return gulp.src([...images]).pipe(gulp.dest(paths.build.base.dir))
  } else {
    return new Promise(function (resolve, reject) {
      resolve()
    })
...
function copySkippedFiles() {
  ...
  if ([...skippedFiles].length) {
    return gulp.src([...skippedFiles]).pipe(gulp.dest(paths.build.base.dir))
  } else {
    return new Promise(function (resolve, reject) {
      resolve()
    })
...
function copyDependencies() {
  for (var k in config.copyDependencies.build) {
    ...
    gulp
      .src(path)
      .pipe(
        gulp.dest(paths.build.base.dir + '/' + config.copyDependencies.build[k])
      )
  }
